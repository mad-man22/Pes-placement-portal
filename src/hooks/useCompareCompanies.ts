...
export const useCompareCompanies = (shortNames: string[]) => {
    ...
    useEffect(() => {
        ...
        const fetchCompanies = async () => {
            try {
                setLoading(true);
                // Fetching deep nested relations for all sections - SAME AS useCompany.ts
          ...
          `)
                ...
                if (data) {
                    const companiesData = data.map((rawData: any) => {
                ...
                console.error('Error fetching companies for comparison:', err);
